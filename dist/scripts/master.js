...
var app = {
  ...
  cacheDOM: function cacheDOM() {
    ...
    this.docHeight = Math.max(this.body.scrollHeight, this.body.offsetHeight, this.html.clientHeight, this.html.scrollHeight, this.html.offsetHeight); // Fun
...
app.animateFooter = function (e) {
  var dist = window.scrollY + window.innerHeight - (this.aboveFooter.offsetTop + this.aboveFooter.offsetHeight);
  var inView = dist > 0;
  var percent = Math.round(dist / this.footerHeight * 100) / 100;

  if (inView) {
    this.footerItems_container.style.transform = "translateY(".concat(-percent * 50, "px)");
  }
...
app.workOnClick = function (e) {
  var target = this.querySelector('.col--expandable');
  var coords = findFixedPos(target);
  TweenMax.set(target, {
    x: coords.x,
    y: coords.y,
    width: coords.w,
  ...
  var tl = new TimelineMax().to(target, .25, {
    x: 0,
    ease: Sine.easeInOut
  }).to(target, .25, {
    y: 0,
...
app.listeners = function () {
  ...
  this.works.map(function (work) {
    work.addEventListener('mouseover', _this.workHoverOn);
    work.addEventListener('mouseout', _this.workHoverOff);
  });
};

var sliderImages = [].slice.call(document.querySelectorAll('.slide-in'));

app.checkScroll = function (e) {
  var offset = 200;
  sliderImages.map(function (image) {
    var slideInAt = window.scrollY + window.innerHeight - offset;
...
var findFixedPos = function findFixedPos(element) {
  return {
